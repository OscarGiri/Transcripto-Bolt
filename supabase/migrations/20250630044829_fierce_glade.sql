...
DECLARE
  ...
  v_monthly_limit integer := 150; -- Not used for daily limits but kept for compatibility
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    ...
    -- Set limits based on plan (daily focus)
    CASE v_plan_type
      WHEN 'pro' THEN
    ...
    -- Get current daily usage for authenticated user
  ...
  ELSE
    -- For visitors, use visitor_id (daily limit only)
    SELECT COALESCE(SUM(ut.count), 0) INTO v_daily_usage
    FROM usage_tracking ut
  ...
  SELECT 
    ...
    (v_daily_usage < v_daily_limit) as can_perform_action,
