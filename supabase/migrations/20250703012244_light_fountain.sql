/*
  # User Subscriptions and Usage Tracking Schema

  1. New Tables
    - `user_subscriptions`
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `plan_type` (text, enum: free, pro, team)
    ...
    - `usage_tracking`
      ...
      - `action_type` (text, enum: video_analysis, api_call, export_pdf, export_docx, translation)
      - `date` (date)
      - `count` (integer)
      - `metadata` (jsonb)
      - `created_at` (timestamptz)
  ...
  3. Functions
    ...
    - `increment_usage()` - Track usage with upsert logic
    - `create_default_subscription()` - Auto-create free subscription for new users
