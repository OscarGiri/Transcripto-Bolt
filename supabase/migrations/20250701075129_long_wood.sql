...
-- RLS Policies for user_subscriptions (with safe creation)
DO $$
BEGIN
...
-- RLS Policies for usage_tracking (with safe creation)
DO $$
BEGIN
  -- Drop existing policies if they exist
...
-- Trigger to create default subscription for new users (safe creation)
DO $$
BEGIN
...
-- Add updated_at triggers (safe creation)
DO $$
BEGIN
