/*
  # Enhanced User Management and Usage Tracking

  1. New Tables
    - `user_subscriptions`
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `plan_type` (text, check constraint for 'free', 'pro', 'team')
      - `status` (text, check constraint for subscription statuses)
  ...
  2. Security
    ...
    - Add policies for anonymous visitor tracking
    - Add service role policies for system management

  3. Functions
    - `get_user_subscription()` - Get current user subscription
    - `check_usage_limit()` - Check if user can perform action
    - `increment_usage()` - Track usage with conflict resolution
    - `create_default_subscription()` - Auto-create free subscription
...
DO $$
BEGIN
  -- Drop existing policies for usage_tracking
  ...
  DROP POLICY IF EXISTS "Anonymous users can insert visitor usage" ON usage_tracking;
  DROP POLICY IF EXISTS "System can manage all usage" ON usage_tracking;
